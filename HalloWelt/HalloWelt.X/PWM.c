...
void pwm_init(void){
    //PR2 = (_XTAL_FREQ/(PWM_freq*4)) - 1; //Setting the PR2 formulae using Datasheet // Makes the PWM work in 5KHZ
    PR2 = 100;
...
void pwm_duty(unsigned int duty)
{
      if(duty<1023)
  {
    ...
    //duty = ((float)duty/1023)*(_XTAL_FREQ/(PWM_freq)); // On reducing //duty = (((float)duty/1023)*(1/PWM_freq)) / ((1/_XTAL_FREQ) * TMR2PRESCALE);
    //DC3B0 = duty & 1;
    //DC3B1 = duty & 2;
